...
启动 Geth 时，可以向其提供许多标志和命令来影响节点的行为。通常，将这些选项配置在文件中比每次启动节点时都在命令行中输入要方便得多。这可以通过一个简单的 shell 脚本来启动 Geth 来实现。

此外，还有一些配置选项无法通过命令行访问，但可以通过向 Geth 提供配置文件来调整。这样就可以访问影响 Geth 某些内部组件行为的底层配置。

### shell 脚本

编写用于启动 Geth 节点的 Shell 脚本的好处在于，它更容易复用，而且您无需记住大量用于设置节点特定行为的语法。这对于运行多个具有各自特定配置的节点尤其有用。

要创建 Shell 脚本，请将 Geth 启动命令保存在一个 Shell 文件中，并在其前面添加 `#!/bin/bash`。该文件的内容可能如下所示:
```s
  #!/bin/bash
...
您也可以通过配置文件来调整更深层的配置。配置文件比 Shell 脚本更复杂，但它可以触及 Geth 内部配置结构中无法通过命令行界面访问的部分。

配置文件应该是一个 `.toml` 文件。创建配置文件的一种便捷方法是让 Geth 为您创建一个配置文件并将其用作模板。为此，可以使用 `dumpconfig` 命令，将结果保存到 `.toml` 文件中。请注意，您还需要在命令行中为公共测试网（例如 Sepolia 或 Geoerli）明确提供 network_id:
```s
  geth --sepolia dumpconfig > geth-config.toml
```

您可以修改此文件中的值，然后在启动时将其传递给 Geth，以便节点能够按照您的意愿进行配置。要覆盖配置文件中指定的选项，请在命令行中指定相同的选项。

要使用 `geth-config.toml` 中定义的配置运行 Geth，请将配置文件路径传递给 `--config` 参数。`network_id` 不会从配置文件中保存，它必须在启动时在命令行中显式定义，例如:
```s
...
使用 `dumpconfig` 创建的配置文件包含以下信息（此示例适用于 Sepolia 测试网 - 主网和其他网络配置会略有不同）:
```s
[Eth]
NetworkId = 11155111
SyncMode = "snap"
