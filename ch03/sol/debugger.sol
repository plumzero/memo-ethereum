...
contract bet {
    ...
    event Staked(address indexed user, string information);
    event Opened(string information);
    ...
    function stake(bool flag) public payable returns (bool) {
        ...
        if (flag) {
            inBig.push(p);
            totalBig += p.amount;
            emit Staked(msg.sender, "in big");
        } else {
            inSmall.push(p);
            totalSmall += p.amount;
            emit Staked(msg.sender, "in small");
        }
