...
执行该命令后，会在家目录下创建一个 .lighthouse 目录，里面有一个 hoodi 目录。
...
```s
  clef --keystore ~/geth-hoodi/keystore --configdir ~/.clef --chainid $CHAIN_ID --http
```
...
```s
  cd ~
  geth --hoodi --datadir geth-hoodi --syncmode snap --http --http.port 8545 --http.api eth,net --authrpc.addr localhost --authrpc.port 8551 --authrpc.vhosts localhost --authrpc.jwtsecret geth-hoodi/jwtsecret --signer=.clef/clef.ipc
```
